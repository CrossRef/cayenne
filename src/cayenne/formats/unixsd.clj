...
(defn parse-update-date [oai-record]
  (xml/xselect1 oai-record :> "crm-item" [:= "name" "last-update"] :text))
...
(defn unixsd-record-parser
  [oai-record]
  (let [work (unixref/unixref-record-parser oai-record)
        primary-id (unixref/parse-primary-id oai-record)]
    (-> work
        (itree/add-relation :publisher (parse-publisher oai-record))
        (itree/add-relation :updated (parse-update-date oai-record))
